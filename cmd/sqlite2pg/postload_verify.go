...
func determineVerify(mode verifyMode, in io.Reader, out io.Writer) bool {
	...
	if f, ok := in.(*os.File); ok && !term.IsTerminal(int(f.Fd())) {
		// Print the prompt on this path too, and echo the answer the pipe
		// supplied verbatim (trimmed, but not case-folded — the case
		// fold is only for the y/yes comparison), so a
		// `sqlite2pg load < answers.txt` transcript records what was
		// asked and answered rather than showing nothing at all
