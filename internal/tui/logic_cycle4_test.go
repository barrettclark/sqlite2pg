...
// double precision stayed on offer. (Cycle 5 / issue #156: the
// normalization is now gated on the column having numeric SQLite
// affinity, so these cases pass "REAL" as the declared type.)
	...
	for _, c := range cases {
		...
		if c.wantValid {
			if display != c.wantResult {
				t.Errorf("previewValueForType(%q, %q, REAL) display = %q, want %q", c.value, c.targetType, display, c.wantResult)
			}
			if transform != "numeric_text_to_integer" {
				t.Errorf("previewValueForType(%q, %q, REAL) transform = %q, want numeric_text_to_integer", c.value, c.targetType, transform)
			}
		}
	}
}
